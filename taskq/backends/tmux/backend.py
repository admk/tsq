...
class TmuxBackend(BackendBase):
    ...
    def __init__(self, name, config):
        ...
        self.tmux_config_file = Path(os.path.expanduser(
            ...
            or str(self.state_dir / 'tmux.conf')
        ))
    ...
    def _tmux_config_content(self):
        history_limit = self.config.get('history_limit', 100000)
        return '\n'.join([
            '# Generated by taskq. Local ~/.tmux.conf is not loaded for taskq.',
            'set -g prefix C-b',
    ...
    def _ensure_tmux_config(self):
        self.tmux_config_file.parent.mkdir(parents=True, exist_ok=True)
        content = self._tmux_config_content()
        try:
            if self.tmux_config_file.read_text(encoding='utf-8') == content:
                return
        except FileNotFoundError:
            pass
        tmp = self.tmux_config_file.with_suffix(
            self.tmux_config_file.suffix + '.tmp')
        tmp.write_text(content, encoding='utf-8')
        os.replace(tmp, self.tmux_config_file)

    def _tmux_global_args(self):
        return ['-f', str(self.tmux_config_file)] + self._socket_args()
    ...
    def _source_tmux_config(self):
        self._tmux('source-file', str(self.tmux_config_file), check=False)
    ...
    def backend_info(self):
        info = {
            ...
            'tmux_config': str(self.tmux_config_file),
